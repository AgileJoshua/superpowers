...
function init(player, callback) {
    ...
    player.createActor = function (name, parentActor, options) { return new window.Sup.Actor(name, parentActor, options); };
    player.createComponent = function (type, actor, config) {
        if (type === "Behavior") {
            ...
            if (actorComponentTypesByName[type] == null) {
                ...
                var parts = SupCore.system.api.contexts["typescript"].plugins[type].exposeActorComponent.className.split(".");
...
function start(player, callback) {
    ...
    var line = getLineCounts(jsGlobals.script) + 2;
...
function loadAsset(player, entry, callback) {
    ...
    player.getAssetData("assets/" + entry.storagePath + "/script.ts", "text", function (err, script) {
